...
class RecycleViewAdapterMultiple(
    val items: MutableList<TabMenuItem.Data>,
    private val currentItemStatus: (Int,Boolean) -> Unit

) : RecyclerView.Adapter<RecycleViewAdapterMultiple.ViewHolder>() {
...
//            if (prevIndex != -1) {
//                currentItemStatus(position,!items[position].isActive)
//            }
        }
...
//                onActiveItemChange(activeIndex)
            ...
            currentItemStatus(position,it)
            items[position].isActive = it
//            items[position]. = it
        }
    }
